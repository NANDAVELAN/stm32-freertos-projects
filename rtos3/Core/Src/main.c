	...
	while(1)
	    {
	        if(osMessageQueueGet(
	                ...
	                osWaitForever) == osOK)
	        {
	            printf(" increment count if push button presses = %lu\r\n", receivedData);
	        }
	    }
}
...
int main(void)
{
  ...
  /* USER CODE BEGIN RTOS_THREADS */
  	...
  	senderTaskHandle = osThreadNew(SenderTask,
                                   NULL,
                                   &senderTask_attributes);

    receiverTaskHandle = osThreadNew(ReceiverTask,
                                     NULL,
